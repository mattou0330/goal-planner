...
export const timerRecordsApi = {
  ...
  async getMonthlyStats(year, month) {
    ...
    return withRetry(async () => {
      ...
      const { data, error } = await supabase
        .from("timer_records")
        .select("duration, type, date, goal_id")
        .gte("date", startDate)
        .lte("date", endDate)
  ...
  async getUserStats() {
    ...
    return withRetry(async () => {
      ...
      const [bigGoalsCount, smallGoalsCount, recordsCount, timerRecordsCount] = await Promise.all([
        supabase.from("big_goals").select("id", { count: "exact" }).eq("user_id", user.id),
        supabase.from("small_goals").select("id", { count: "exact" }).eq("user_id", user.id),
        supabase.from("records").select("id", { count: "exact" }).eq("user_id", user.id),
        supabase.from("timer_records").select("id", { count: "exact" }).eq("user_id", user.id),
      ])
